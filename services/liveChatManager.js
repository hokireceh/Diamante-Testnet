...
class LiveChatManager {
    ...
    startChat(adminId, userId, userName = null, geolocation = null) {
        const chat = {
        ...
        if (!this.chatHistory.has(userIdStr)) {
            this.chatHistory.set(userIdStr, []);
        }
    ...
    async addGeolocationToChat(adminId, userContext) {
        const chat = this.activeChats.get(adminId.toString());
        if (!chat) return null;

        const geoData = await geolocationService.getUserGeolocation(userContext);
        if (geoData) {
            chat.geolocation = geoData;
            logger.debug(`Geolocation added for user ${chat.userId}: ${geoData.country}`);
        }
    ...
    getChatHistory(userId) {
        return this.chatHistory.get(userId.toString()) || [];
    }

    addChatMessage(userId, message, sender = 'user', adminName = null) {
        const userIdStr = userId.toString();
        if (!this.chatHistory.has(userIdStr)) {
            this.chatHistory.set(userIdStr, []);
        }
        ...
        this.chatHistory.get(userIdStr).push(msg);
    }
    ...
    async queueUserMessage(userId, message, ctx) {
        ...
        this.addChatMessage(userId, message, 'user', ctx.from?.first_name || 'User');
    ...
    async sendReply(bot, adminId, userId, message, options = {}) {
        ...
        try {
            ...
            this.addChatMessage(userId, message, 'admin', adminName);
