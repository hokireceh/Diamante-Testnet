...
class ValidationHelper {
    ...
    static validateEntities(entities) {
        ...
        for (const entity of entities) {
            // Each entity must have: type, offset, length
            if (!entity.type || typeof entity.offset !== 'number' || typeof entity.length !== 'number') {
                return { 
                    valid: false, 
                    error: `Invalid entity structure. Must have type, offset, length.`
                };
            }

            // Validate entity type - support semua Telegram text formatting
            const validTypes = [
                'bold', 'italic', 'underline', 'strikethrough', 
                'blockquote', 'code', 'pre', 'text_link', 
                'text_mention', 'spoiler', 'custom_emoji'
            ];

            if (!validTypes.includes(entity.type)) {
                return { 
                    valid: false, 
                    error: `Invalid entity type: ${entity.type}`
                };
            }
            ...
            if (entity.offset < 0 || entity.length < 0) {
                return { 
                    valid: false, 
                    error: `Entity offset and length must be non-negative`
                };
            }
        }
