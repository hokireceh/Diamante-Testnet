...
class LiveChatHandler {
    ...
    async startChatWithUser(ctx, userId) {
        ...
        const userContext = userMessages[0]?.from || {};
        ...
        await liveChatManager.addGeolocationToChat(adminId, userContext);
        const chat = liveChatManager.getActiveChat(adminId);
        ...
        if (chat?.geolocation) {
            const geoFormatted = geolocationService.formatGeolocation(chat.geolocation);
            if (geoFormatted) {
                message += `📍 ${geoFormatted}\n`;
            }
        }
        ...
        if (userMessages.length > 0) {
            message += `📬 <b>Pesan dari user (${userMessages.length} msg):</b>\n`;
        ...
        // Notify user that admin is now chatting
        if (this.bot) {
            try {
                await this.bot.telegram.sendMessage(
                    userId,
                    `🟢 <b>Admin Terhubung!</b>\n\n👨‍💼 ${adminName} sedang online untuk chat dengan Anda.\n\nGunakan /chat untuk lihat percakapan!`,
                    { parse_mode: 'HTML' }
                );
            } catch (error) {
                logger.debug(`Failed to notify user: ${error.message}`);
            }
        }
